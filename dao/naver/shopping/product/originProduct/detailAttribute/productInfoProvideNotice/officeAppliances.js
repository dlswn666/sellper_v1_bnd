 ...
 * @param {string} data.certificationType - KC 인증정보 (｢전기용품 및 생활용품 안전관리법｣에 따른 안전인증·안전확인·공급자적합성확인대상 제품 및 ｢전파법｣에 따른 적합인증·적합등록 대상 기자재에 한함)
 * @param {string} data.ratedVoltage - 정격전압
 * @param {string} data.powerConsumption - 소비전력
 * @param {string} data.energyEfficiencyRating - 에너지소비효율등급
 *
 * @param {object} data.releaseDate - 출시일 ( <'yyyy-MM' 형식 입력> (동일 모델의 출시연월) )
 * @param {string} data.releaseDate.year - 출시연도
 * @param {string} data.releaseDate.month - 출시월
 * @param {number} data.releaseDate.monthValue - 출시월 값
 * @param {boolean} data.releaseDate.leapYear - 윤년 여부
 *
 ...
 * @param {string} data.weight - 무게 ( <= 200 characters ) 무게는 노트북 등 휴대형 기기에 한함 ( <해당 사항이 없으면 이 요소를 삭제하고 전송합니다> )
 ...
 * @returns {Object}
 */
export function createOfficeAppliances(data) {
    const {
        returnCostReason,
        noRefundReason,
    ...
    return {
        ...
        releaseDate: {
            ...
            monthValue: releaseDate.monthValue,
            leapYear: releaseDate.leapYear,
        },
        releaseDateText,
        manufacturer,
        size,
