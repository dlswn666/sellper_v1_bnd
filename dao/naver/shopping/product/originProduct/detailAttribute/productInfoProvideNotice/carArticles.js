 ...
 * @param {string} data.certificationType - KC 인증정보 (｢자동차관리법｣에 따른 부품자기인증 대상 자동차부품 ｢전기용품 및 생활용품 안전관리법｣에 따른 안전인증·안전확인·공급자적합성확인대상제품 및 ｢전파법｣에 따른 적합인증·적합등록 대상 기자재에 한함)
 * @param {string} data.caution - 연료절감장치에 한함
 * @param {string} data.manufacturer - 제조사
 * @param {string} data.size - 사이즈
 * @param {string} data.applyModel - 적용 차종
 * @param {string} data.warrantyPolicy - 품질 보증 기준
 * @param {string} data.roadWorthyCertification - 검사합격증 번호 (｢대기환경보전법｣에 따른 첨가제·촉매제에 한함)
 * @param {string} data.afterServiceDirector - A/S 책임자 및 전화 번호
 *
 * @returns {Object}
 */
export function createCarArticles(data) {
    const {
        returnCostReason,
        noRefundReason,
